...
Another way to save the data is to a spreadsheet file in the drive you have as a user on XSEDE. Here is how you can do that:

```{r, eval=FALSE}
write.table(sp_out, file="~/RAISE_Team1/SpeciesData.tsv", sep="\t")
```
...
```{r, eval=F}
spDF <- read.table(file="~/RAISE_Team1/SpeciesData.tsv")
```
